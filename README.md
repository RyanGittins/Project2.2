...
Overall the traffic flow of this intersection is improved because of the implementation mentioned above.  As mentioned, depending on the destination of a given car we can allow 4 cars to be in the intersection at one time.  However, if there are 3 cars in the intersection and none of them will be turning right or will be immediately exiting we will not allow anymore cars to enter the intersection.  This allows the intersection to handle cars at near maximum capacity while ensuring we do not have a deadlock.
